...
args.output_dir = os.path.join("result", "geo", args.selective_annotation_method + "_" + str(args.annotation_size) + "_"+ str(args.seed))
...
while f:
    ...
    for file in prompt_files:
        ...
        if not os.path.isfile(os.path.join(output_dir, file)):
            ...
            execution_count += 1
            try:
                codex_execution(key=cur_key, 
                                output_path=os.path.join(output_dir, file),
                                prompt_path=os.path.join(prompt_cache_dir, file))
                print('prompt_path', os.path.join(prompt_cache_dir, file))
            except Exception as e:
                print(e)
                time.sleep(3)
preds = []
