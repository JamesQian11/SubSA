...
if __name__=='__main__':
    set_seed(args.seed)
    n1 = model_size =  args.model_name.split('-')[-1] 
    # args.output_dir = os.path.join("result", args.task_name, args.selective_annotation_method + "_" + str(args.seed))
    args.output_dir = os.path.join("result", args.task_name, args.selective_annotation_method + "_"+ args.prompt_retrieval_method + "_" + str(args.annotation_size) + "_"+ str(args.seed)+"_"+ n1)
    ...
    if args.task_name in ['mnli','rte','sst5','mrpc','dbpedia_14','hellaswag','xsum','nq','ag_news','amazon','sst2','trec']:
        ...
        print('first_phase_selected_indices', first_phase_selected_indices)
        ...
        golds = []
        preds = []
        if not args.task_name in ['hellaswag','xsum','np']:
            all_labels = []
            label_to_digit = {}
                ...
                if not os.path.isfile(os.path.join(output_dir,file)):
                    ...
                    if args.task_name == 'hellaswag':
                        ...
                        gen_tokens = inference_model.generate(input_ids, do_sample=False, temperature=0.7,
                                                              ...
                                                              output_scores=True, return_dict_in_generate=True# add pad token id here
