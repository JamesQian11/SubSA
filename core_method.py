...
def selective_annotation(args,**kwargs):
    ...
    elif args.selective_annotation_method=='submodular': 
        embeddings = kwargs['embeddings'] 
        n = len(embeddings)
        index = np.arange(0,n)                                              
        submod_function = submodular_function.SubSA(index=index, similarity_kernel=lambda a, b:cossim_np(embeddings[a], embeddings[b]))               
        submod_optimizer = submodular_optimizer.Greedy(index=index, budget=round(args.annotation_size))            
        selected_indices = submod_optimizer.select(gain_function=submod_function.calc_gain, update_state=submod_function.update_state)
